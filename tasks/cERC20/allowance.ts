...
task("allowance", "Get user allowance")
  ...
  .setAction(async ({ signeraddress, tokenaddress, spenderaddress, address }, hre) => {
    ...
    const encryptedAllowance = await cerc20.allowance(address, spenderaddress);

    console.info("Encrypted allowance: ", encryptedAllowance);

    const userAllowance = await instance.reencrypt(
      encryptedAllowance, // the encrypted balance
    ...
    console.log("Decrypted allowance: ", userAllowance);
  });
